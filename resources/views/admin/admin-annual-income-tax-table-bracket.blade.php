                        ...
                        <div class="col-12">
                            <div class="breadcrumb-wrapper col-12">
                                <ol class="breadcrumb p-0 mb-0">
                                    ...
                                    <li class="breadcrumb-item active">Annual Income Tax Table List
                                    </li>
                                </ol>
                            </div>
                        </div>
                        ...
                        <div class="col-md-12">
                            <div class="card">
                                <div class="card-header">
                                    <h4 class="card-title">Annual Income Tax Table</h4>
                                </div>
        ...
        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal" role="document">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <h5 class="modal-title white-color">Annual Income Tax Information</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <i class="bx bx-x"></i>
                    </button>
                </div>
                <div class="modal-body">
                    <input type="hidden" id="AnnualIncomeTaxID" value="0" readonly>
                        <div class="row">
                            <div class="col-md-6">
    ...
    <div id="upload-modal" class="modal fade text-left w-100" role="dialog" aria-labelledby="myModalLabel16" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal" role="document">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <h5 class="modal-title">Annual Income Tax Excel Uploader</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <i class="bx bx-x"></i>
                    </button>
                </div>
                <div class="modal-body">
                    <div class="row">
                          <h5>Browse & select Annual Income Tax excel file:</h5>
                         <fieldset class="form-group">
...
<script type="text/javascript">
    ...
    function getRecordList(vPageNo){
      ...
      vLimit=100;
        ...
        $.ajax({
            ...
            url: "{{ route('get-annual-income-tax-table-bracket-list') }}",
            dataType: "json",
            success: function(data){
                 total_rec=data.TotalRecord;
                LoadRecordList(data.AnnualIncomeTaxBracketList);
                 if(total_rec>0){
                     CreateAnnualIncomeTaxPaging(total_rec,vLimit);  
                     if(total_rec>vLimit){
                        $("#divPaging").show(); 
    ...
    function CreateAnnualIncomeTaxPaging(vTotalRecord,vLimit){
    ...
    function LoadRecordRow(vData){
        ...
        tdRateOnExcessPercentage = "<span>" + vData.RateOnExcessPercentage + " %</span>";
        ...
        tblList.rows().every(function ( rowIdx, tableLoop, rowLoop ) {
            ...
            if(rowData[0] == vData.ID){
                ...
                curData[4] = tdFixTax;
                curData[5] = tdRateOnExcessPercentage;
                curData[6] = tdYear;
                curData[7] = tdStatus;
        ...
        if(vRecordID > 0){
            $.ajax({
                ...
                data: {
                    _token: '{{ csrf_token() }}',
                    Platform: "{{ config('app.PLATFORM_ADMIN') }}",
                    AnnualIncomeTaxID: vRecordID
                },
                url: "{{ route('get-annual-income-tax-table-info') }}",
                dataType: "json",
                success: function(data){

                    if(data.Response =='Success' && data.AnnualIncomeTaxInfo != undefined){

                        Clearfields();

                        $("#AnnualIncomeTaxID").val(data.AnnualIncomeTaxInfo.ID);                        
                        $("#SalaryTo").val(FormatDecimal(data.AnnualIncomeTaxInfo.RangeTo,2));
                        $("#SalaryFrom").val(FormatDecimal(data.AnnualIncomeTaxInfo.RangeFrom,2)); 
                        $("#FixTax").val(FormatDecimal(data.AnnualIncomeTaxInfo.FixTax,2));

                        $("#RateonExcessPercent").val(data.AnnualIncomeTaxInfo.RateOnExcessPercentage);
                        $("#Year").val(data.AnnualIncomeTaxInfo.Year).change();
                        $("#Status").val(data.AnnualIncomeTaxInfo.Status).change();
    ...
    function SaveRecord(){
        ...
        var vAnnualIncomeTaxID = $("#AnnualIncomeTaxID").val();
       ...
       if(vFixTax=="") {
         showHasErrorMessage('FixTax','Enter annual income tax for fix tax.');
         return;  
       }

        if(vRateonExcessPercent=="") {
         showHasErrorMessage('RateonExcessPercent','Enter annual income rate for excess percentage.');
         return;  
       }
       ...
       // SAVE DATA
        $.ajax({
            ...
            data: {
                _token: '{{ csrf_token() }}',
                Platform: "{{ config('app.PLATFORM_ADMIN') }}",
                AnnualIncomeTaxID: $("#AnnualIncomeTaxID").val(),
            ...
            url: "{{ route('do-save-annual-income-tax-table')}}",
            dataType: "json",
            success: function(data){
                ...
                if(data.Response =='Success'){
                    ...
                    LoadRecordRow(data.AnnualIncomeTaxInfo);
                }else{
 ...
 function GenerateExcel(){
    $.ajax({
          ...
          url: "{{ route('get-excel-annual-income-tax-list') }}",
          dataType: "json",
          success: function(data){

              if(data.Response=="Success"){
                   resultquery=data.AllAnnualIncomeTaxList;
                   ShowGeneratedExcel();
              }
  ...
  function ShowGeneratedExcel(){
         ...
         // Excel Headers
          var xlsHeader = [
                            " Salary From "," Salary To "," Fix Tax ", 
                            " Rate On Excess % "," Year "," Status "                            
                          ];
