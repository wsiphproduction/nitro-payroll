                        ...
                        <div class="col-12">
                            <div class="breadcrumb-wrapper col-12">
                                <ol class="breadcrumb p-0 mb-0">
                                    ...
                                    <li class="breadcrumb-item active"> Employee Earning Taxable Report
                                    </li>
                                </ol>
                            </div>
                        </div>
                        ...
                        <div class="col-md-12">
                            <div class="card">
                                <div class="card-header">
                                    <h4 class="card-title"> Employee Earning Taxable Report</h4>
                                </div>
                                            ...
                                            <table id="tblList" class="table zero-configuration complex-headers border alt-background">
                                                <thead>
                                                        ...
                                                        @foreach($IncomeDeductionTypeList as $idrow)
                                                            @if($idrow->Type == "Taxable Income" && $idrow->ID >= 99 && $idrow->ID <= 148)
                                                        <th style="min-width: 110px;">{{ $idrow->Name }}</th>
                                                            @endif
                                                        @endforeach
                                                        <th style="min-width: 130px;">ADJ. FROM PREV. PAYROLL</th>
...
<script type="text/javascript">
    ...
    $('.searchtext').on('keypress', function (e) {
     ...
     function getRecordList(vPageNo){
        ...
        $.ajax({
            ...
            url: "{{ route('get-admin-employee-income-taxable-list-by-filter') }}",
            dataType: "json",
            success: function(data){
                ...
                if(data.Response=='Success'){
                    ...
                    if(total_rec>vLimit){
                        ...
                        LoadRecordList(data.PayrollTransactionEmployeeIncomeTaxableList);
                    }else{
   ...
   function LoadRecordRow(vData){
            ...
            tdAdjustmentPrevPayroll = "<span class='font-normal float_right'>" + FormatDecimal(vData.AdjustmentPrevPayroll,2) + "</span>";
            ...
            tblList.rows().every(function ( rowIdx, tableLoop, rowLoop ) {
                ...
                if(rowData[0] == vData.ID){
                    ...
                    @php($intCol = $intCol + 1)
                    curData[{{$intCol}}] = tdAdjustmentPrevPayroll;
                    @php($intCol = $intCol + 1)
   ...
   function Print(){
    ...
    // showHasSuccessMessage('Printing  employee other earning taxable list base on paging batch no.' + vPageBatchNO);  
    
    setTimeout(function (){     
       
       if(vFilterType=='All'){
        window.open('{{config('app.url')}}admin-employee-income-taxable-print-report?PayrollPeriodID=' +vPayrollPeriodID +'&FilterType='+ vFilterType, '_blank');
       }

        if(vFilterType=='Location'){
        window.open('{{config('app.url')}}admin-employee-income-taxable-print-report?PayrollPeriodID=' +vPayrollPeriodID +'&FilterType='+ vFilterType +'&BranchID='+ vBranchID, '_blank');
       }

        if(vFilterType=='Division'){
         window.open('{{config('app.url')}}admin-employee-income-taxable-print-report?PayrollPeriodID=' +vPayrollPeriodID +'&FilterType='+ vFilterType +'&DivisionID='+ vDivisionID, '_blank');
       }

        if(vFilterType=='Department'){
         window.open('{{config('app.url')}}admin-employee-income-taxable-print-report?PayrollPeriodID=' +vPayrollPeriodID +'&FilterType='+ vFilterType +'&DepartmentID='+ vDepartmentID, '_blank');
       }

        if(vFilterType=='Section'){
       window.open('{{config('app.url')}}admin-employee-income-taxable-print-report?PayrollPeriodID=' +vPayrollPeriodID +'&FilterType='+ vFilterType +'&SectionID='+ vSectionID, '_blank');
       }

        if(vFilterType=='Job Type'){
       window.open('{{config('app.url')}}admin-employee-income-taxable-print-report?PayrollPeriodID=' +vPayrollPeriodID +'&FilterType='+ vFilterType +'&JobTypeID='+ vJobTypeID, '_blank');
       }

        if(vFilterType=='Employee'){
       window.open('{{config('app.url')}}admin-employee-income-taxable-print-report?PayrollPeriodID=' +vPayrollPeriodID +'&FilterType='+ vFilterType +'&EmployeeID='+ vEmployeeID, '_blank');
       }
    ...
    function GenerateExcel(){
      ...
      showHasSuccessMessage('Please wait while generating Employee Other Earning Taxable excel file.');  
        ...
        $.ajax({
              ...
              url: "{{ route('get-excel-employee-other-earning-taxable-list') }}",
              dataType: "json",
              success: function(data){

                  if(data.Response=="Success"){
                       resultquery=data.EmployeeOtherEarningTaxableExcelList;
                       if (total_rec>0){                          
                          ShowGeneratedExcel();
                       }else{
                        showHasErrorMessage('','No record(s) found base on search criteria.');
         ...
         // Excel Headers
          var xlsHeader = [
                            ...
                            @foreach($IncomeDeductionTypeList as $idrow)
                                ...
                                @endif
                            @endforeach
                            "{{ strtoupper('Adjustment From Previous Payroll') }}", 
                            "{{ strtoupper('Status') }}"
                          ];
            ...
            @foreach($IncomeDeductionTypeList as $idrow)
                ...
                @endif
            @endforeach
            var dblAdjustmentPrevPayroll = 0;
                  ...
                  $.each(value, function(ind, val) {
                    ...
                    if(
                        ...
                        if(
                            ...
                            if(ind == "AdjustmentPrevPayroll"){
                                dblAdjustmentPrevPayroll = parseFloat(dblAdjustmentPrevPayroll) + parseFloat(val);
                            }
            ...
            @foreach($IncomeDeductionTypeList as $idrow)
                ...
                @endif
            @endforeach
            innerRowData.push(dblAdjustmentPrevPayroll);
            createXLSLFormatObj.push(innerRowData);
            intRowCnt = intRowCnt + 1;
            ...
            XLSX.utils.book_append_sheet(wb, ws,"Other Earning Taxable Sheet");

            /* generate file and download */
            const wbout = XLSX.write(wb, { type: "array", bookType: "xlsx" });
            saveAs(new Blob([wbout], { type: "application/octet-stream" }), "Other-Earning-Taxable-Report.xlsx");
