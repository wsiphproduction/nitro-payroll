                        ...
                        <div class="col-12">
                            <div class="breadcrumb-wrapper col-12">
                                <ol class="breadcrumb p-0 mb-0">
                                    ...
                                    <li class="breadcrumb-item active"> Employee Loan Deduction Report
                                    </li>
                                </ol>
                            </div>
                        </div>
                        ...
                        <div class="col-md-12">
                            <div class="card">
                                <div class="card-header">
                                    <h4 class="card-title"> Employee Loan Deduction Report </h4>
                                </div>
...
<script type="text/javascript">
    ...
    $('.searchtext').on('keypress', function (e) {
     ...
     function getRecordList(vPageNo){
        ...
        $.ajax({
            ...
            url: "{{ route('get-admin-employee-loan-deduction-list-by-filter') }}",
            dataType: "json",
            success: function(data){
                ...
                if(data.Response=='Success'){
                    ...
                    if(total_rec>vLimit){
                        ...
                        LoadRecordList(data.PayrollTransactionEmployeeLoanDeductionList);
                    }else{
   ...
   function LoadRecordRow(vData){
            ...
            @foreach($LoanTypeList as $lrow)
                @if($lrow->ID <= 25)
                    tdLoan{{$lrow->ID}} = "<span class='font-normal'>" +  FormatDecimal(vData.Loan{{$lrow->ID}},2) + "</span>";
                @endif
            @endforeach
   ...
   function Print(){
    ...
    setTimeout(function (){     
       ...
       if(vFilterType=='All'){
        window.open('{{config('app.url')}}admin-employee-loan-print-report?PayrollPeriodID=' +vPayrollPeriodID +'&FilterType='+ vFilterType, '_blank');
       }

        if(vFilterType=='Location'){
        window.open('{{config('app.url')}}admin-employee-loan-print-report?PayrollPeriodID=' +vPayrollPeriodID +'&FilterType='+ vFilterType +'&BranchID='+ vBranchID, '_blank');
       }

        if(vFilterType=='Division'){
         window.open('{{config('app.url')}}admin-employee-loan-print-report?PayrollPeriodID=' +vPayrollPeriodID +'&FilterType='+ vFilterType +'&DivisionID='+ vDivisionID, '_blank');
       }

        if(vFilterType=='Department'){
         window.open('{{config('app.url')}}admin-employee-loan-print-report?PayrollPeriodID=' +vPayrollPeriodID +'&FilterType='+ vFilterType +'&DepartmentID='+ vDepartmentID, '_blank');
       }

        if(vFilterType=='Section'){
       window.open('{{config('app.url')}}admin-employee-loan-print-report?PayrollPeriodID=' +vPayrollPeriodID +'&FilterType='+ vFilterType +'&SectionID='+ vSectionID, '_blank');
       }

        if(vFilterType=='Job Type'){
       window.open('{{config('app.url')}}admin-employee-loan-print-report?PayrollPeriodID=' +vPayrollPeriodID +'&FilterType='+ vFilterType +'&JobTypeID='+ vJobTypeID, '_blank');
       }

        if(vFilterType=='Employee'){
       window.open('{{config('app.url')}}admin-employee-loan-print-report?PayrollPeriodID=' +vPayrollPeriodID +'&FilterType='+ vFilterType +'&EmployeeID='+ vEmployeeID, '_blank');
       }
    ...
    if(total_rec<=0){
      ...
      showHasSuccessMessage('Please wait while generating Employee Loan Deduction excel file.');           
        ...
        $.ajax({
              ...
              url: "{{ route('get-excel-employee-loan-deduction-list') }}",
              dataType: "json",
              success: function(data){

                  if(data.Response=="Success"){
                       resultquery=data.EmployeeLoanDeductionExcelList;
                       if (total_rec>0){                          
                          ShowGeneratedExcel();
                       }else{
                        showHasErrorMessage('','No record(s) found base on search criteria.');
    ...
    function ShowGeneratedExcel(){
          ...
          xlsReportHeader = [
                            "Employee Loan Deduction Report",
                            "",
                            ""
                          ];
          createXLSLFormatObj.push(xlsReportHeader);
         ...
         // Excel Headers
          var xlsHeader = [
                    ...
                    if(ind == "EmployeeNo" ||
                        ...
                        if(
                            ...
                            @foreach($LoanTypeList as $lrow)
                                @if($lrow->ID <= 25)
                                    if(ind == "Loan{{$lrow->ID}}"){
                                        dblLoan{{$lrow->ID}} = parseFloat(dblLoan{{$lrow->ID}}) + parseFloat(val);
                                    }
                                @endif
                            @endforeach
            ...
            @foreach($LoanTypeList as $lrow)
                @if($lrow->ID <= 25)                    
                    innerRowData.push(dblLoan{{$lrow->ID}});
                @endif
            @endforeach
            ...
            XLSX.utils.book_append_sheet(wb, ws,"Loan Deduction Sheet");

            /* generate file and download */
            const wbout = XLSX.write(wb, { type: "array", bookType: "xlsx" });
            saveAs(new Blob([wbout], { type: "application/octet-stream" }), "Loan-Deduction-Report.xlsx");
