                        ...
                        <div class="col-12">
                            <div class="breadcrumb-wrapper col-12">
                                <ol class="breadcrumb p-0 mb-0">
                                    ...
                                    <li class="breadcrumb-item active">HDMF Table List
                                    </li>
                                </ol>
                            </div>
                        </div>
                        ...
                        <div class="col-md-12">
                            <div class="card">
                                <div class="card-header">
                                    <h4 class="card-title">HDMF Table </h4>
                                </div>
                                            ...
                                            <table id="tblList" class="table zero-configuration complex-headers border alt-background">
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th>SALARY FROM</th>
                                                        <th>SALARY TO</th>
                                                        <th>EMPLOYEE SHARE PERCENTAGE</th>
                                                        <th>EMPLYER SHARE PERCENTAGE</th>
                                                        <th>YEAR</th>
                                                        <th>STATUS</th>
                                                    </tr>
                                                </thead>
        ...
        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal" role="document">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <h5 class="modal-title white-color">HDMF Table Information</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <i class="bx bx-x"></i>
                    </button>
                </div>
                <div class="modal-body">
                    <input type="hidden" id="HDMF_ID" value="0" readonly>
                        <div class="row">
                            <div class="col-md-6">
                    ...
                    <div class="row">
                     ...
                     <div class="row">
                        <div class="col-md-6">
                            <fieldset class="form-group">
                                <label for="EmployeeShare">Employee Share %: <span class="required_field">*</span></label>
                                 <div class="div-percent">
                                     <input id="EmployeeShare" type="text" class="form-control DecimalOnly text-align-right" placeholder="Employee Share" autocomplete="off"><span class='percent-sign'> %</span>
                                </div>  
                            </fieldset>
                        </div>
                         <div class="col-md-6">
                            <fieldset class="form-group">
                                <label for="EmployerShare">Employer Share %: <span class="required_field">*</span></label>
                                <div class="div-percent">
                                    <input id="EmployerShare" type="text" class="form-control DecimalOnly text-align-right" placeholder="Employer Share" autocomplete="off">  <span class='percent-sign'> %</span>
                                </div>                   
                            </fieldset>
                        </div>
                    </div> 
    ...
    <div id="upload-modal" class="modal fade text-left w-100" role="dialog" aria-labelledby="myModalLabel16" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal" role="document">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <h5 class="modal-title white-color">HMDF Excel Uploader</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <i class="bx bx-x"></i>
                    </button>
                </div>
                <div class="modal-body">
                    <div class="row">
                          <h5>Browse & select HMDF table excel file:</h5>
                         <fieldset class="form-group">
...
<script type="text/javascript">
    ...
    function getRecordList(vPageNo){
      ...
      vLimit=100;
        ...
        $.ajax({
            ...
            url: "{{ route('get-hdmf-table-bracket-list') }}",
            dataType: "json",
            success: function(data){
                total_rec=data.TotalRecord;
                LoadRecordList(data.HDMFTableBracketList);
                if(total_rec>0){
                     CreateHDMFTableBracketPaging(total_rec,vLimit);  
                     if(total_rec>vLimit){
                        $("#divPaging").show(); 
    ...
    function CreateHDMFTableBracketPaging(vTotalRecord,vLimit){
    ...
    function LoadRecordRow(vData){
        ...
        tdEmployeeSharePercent = "<span>" + vData.EmployeeSharePercent + " % </span>";
        tdEmployerSharePercent = "<span>" + vData.EmployerSharePercent + " % </span>";
        ...
        tblList.rows().every(function ( rowIdx, tableLoop, rowLoop ) {
            ...
            if(rowData[0] == vData.ID){
                ...
                curData[4] = tdEmployeeSharePercent;
                curData[5] = tdEmployerSharePercent;
                curData[6] = tdYear;
                curData[7] = tdStatus;
        ...
        if(!IsRecordExist){
            ...
            tblList.row.add([
                    ...
                    tdSalaryFrom,
                    tdSalaryTo,
                    tdEmployeeSharePercent,
                    tdEmployerSharePercent,
                    tdYear,
                 tdStatus
                ]).draw();          
        }
    ...
    function EnabledDisbledText(vEnabled){
        ...
        $("#EmployeeShare").attr('disabled', vEnabled); 
        $("#EmployerShare").attr('disabled', vEnabled); 
    ...
    function resetTextBorderToNormal(){
        ...
        $("#EmployeeShare").css({"border":"#ccc 1px solid"}); 
        $("#EmployerShare").css({"border":"#ccc 1px solid"});  
    ...
    function EditRecord(vRecordID,vAllowEdit){
        ...
        if(vRecordID > 0){
            $.ajax({
                ...
                url: "{{ route('get-hdmf-table-bracket-info') }}",
                dataType: "json",
                success: function(data){

                    if(data.Response =='Success' && data.HDMFTableBracketInfo != undefined){

                        Clearfields();

                        $("#HDMF_ID").val(data.HDMFTableBracketInfo.ID);

                        $("#SalaryFrom").val(FormatDecimal(data.HDMFTableBracketInfo.RangeFrom,2)); 
                        $("#SalaryTo").val(FormatDecimal(data.HDMFTableBracketInfo.RangeTo,2));

                        $("#EmployeeShare").val(data.HDMFTableBracketInfo.EmployeeSharePercent);
                        $("#EmployerShare").val(data.HDMFTableBracketInfo.EmployerSharePercent);
                        $("#Year").val(data.HDMFTableBracketInfo.Year).change();
                        $("#Status").val(data.HDMFTableBracketInfo.Status).change();
                        ...
                        $("#EmployeeShare").attr("style", "border: 1px solid rgb(204, 204, 204) !important");
                        $("#EmployerShare").attr("style", "border: 1px solid rgb(204, 204, 204) !important");
    ...
    function SaveRecord(){
        ...
        var vHDMF_ID = $("#HDMF_ID").val();
        var vSalaryFrom = $("#SalaryFrom").val();
        var vSalaryTo = $("#SalaryTo").val();
        var vEmployeeShare = $("#EmployeeShare").val();
        var vEmployerShare = $("#EmployerShare").val();
        ...
        if(vYear=="") {
         showHasErrorMessage('Year','Select HDMF year applied from the list.');
         return;  
       }
        ...
        if(vSalaryFrom=="") {
         showHasErrorMessage('SalaryFrom','Enter HDMF salary range from.');
         return;  
       }

       if(vSalaryTo=="") {
         showHasErrorMessage('SalaryTo','Enter HDMF salary range to.');
         return;  
       }

        if(vEmployeeShare=="") {
         showHasErrorMessage('EmployeeShare','Enter HDMF employee share percentage.');
         return;  
       }

        if(vEmployerShare=="") {
         showHasErrorMessage('EmployerShare','Enter HDMF employer share percentage.');
         return;  
       }
       ...
       // SAVE DATA
        $.ajax({
            ...
            data: {
                ...
                EmployeeShare: $("#EmployeeShare").val(),
                EmployerShare: $("#EmployerShare").val(),
                Year: $("#Year").val(),
                Status: $("#Status").val()
            },
            url: "{{ route('do-hdmf-table-bracket') }}",
            dataType: "json",
            success: function(data){
                ...
                if(data.Response =='Success'){
                    ...
                    LoadRecordRow(data.HDMFTableBracketInfo);
                }else{
                    showHasErrorMessage('',data.ResponseMessage);
                    return;  
                }
 ...
 function GenerateExcel(){
    $.ajax({
          ...
          url: "{{ route('get-excel-hdmf-bracket-list') }}",
          dataType: "json",
          success: function(data){

              if(data.Response=="Success"){
                   resultquery=data.AllHDMFBracketList;
                   ShowGeneratedExcel();
              }
  ...
  function ShowGeneratedExcel(){
         ...
         // Excel Headers
          var xlsHeader = [
                            " Salary From "," Salary To "," Employee Share % ", 
                            " Employer Share % "," Year "," Status "                            
                          ];
            ...
            XLSX.utils.book_append_sheet(wb, ws,"HDMF Bracket List Sheet");

            /* generate file and download */
            const wbout = XLSX.write(wb, { type: "array", bookType: "xlsx" });
            saveAs(new Blob([wbout], { type: "application/octet-stream" }), "All-HDMF-Bracket-Report.xlsx");
