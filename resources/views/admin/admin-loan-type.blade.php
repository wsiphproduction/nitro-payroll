                        ...
                        <div class="col-12">
                            <div class="breadcrumb-wrapper col-12">
                                <ol class="breadcrumb p-0 mb-0">
                                    ...
                                    <li class="breadcrumb-item active">Loan Type List
                                    </li>
                                </ol>
                            </div>
                        </div>
        ...
        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal" role="document">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <h5 class="modal-title white-color">Loan Type Information</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <i class="bx bx-x"></i>
                    </button>
                </div>
                <div class="modal-body">
                    <input type="hidden" id="LoanTypeID" value="0" readonly>
                    <div class="row">
                         <div class="col-md-6">
                         ...
                         <div class="col-md-6">
                            <fieldset class="form-group">
                                <label for="LoanTypeCode">Code: <span class="required_field">*</span></label>
                                <input id="LoanTypeCode" type="text" class="form-control" placeholder="Loan Type Code" autocomplete="off"  maxlength="10">
                            </fieldset>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-12">
                            <fieldset class="form-group">
                                <label for="LoanTypeName">Name: <span class="required_field">*</span></label>
                                <input id="LoanTypeName" type="text" class="form-control" placeholder="Loan Type Name" autocomplete="off">
                            </fieldset>
                        </div>
                    </div>
                       ...
                       <div class="row">
                        <div class="col-md-12">
                            <fieldset class="form-group">
                                <label for="LoanTypeDescription">Description: <span class="required_field">*</span></label>
                                <input id="LoanTypeDescription" type="text" class="form-control" placeholder="Loan Type Description" autocomplete="off">
                            </fieldset>
                        </div>
                    </div>
...
<script type="text/javascript">
    ...
    function getRecordList(vPageNo){
      ...
      vLimit=100;
        ...
        $.ajax({
            ...
            url: "{{ route('get-loan-type-list') }}",
            dataType: "json",
            success: function(data){
                 total_rec=data.TotalRecord;
                 LoadRecordList(data.LoanTypeList);
                 if(total_rec>0){
    ...
    function LoadRecordRow(vData){
       ...
       if(vData.Type == 'Taxable Income'){
            tdLoanType += "<span style='color:green;display:flex;'>Taxable Income</span>";
        }else{
            tdLoanType += "<span style='color:red;display:flex;'> Non-Taxable Income </span>";
        }
        
        tdLoanTypeCode = "<span>" + vData.Code + "</span>";
        tdLoanTypeName = "<span>" + vData.Name + "</span>";
        tdLoanTypeDescription = "<span>" + vData.Description + "</span>";
        tdStatus = "";
        ...
        tblList.rows().every(function ( rowIdx, tableLoop, rowLoop ) {
            ...
            if(rowData[0] == vData.ID){
                ...
                curData[3] = tdLoanType;
                curData[4] = tdLoanTypeName;
                curData[5] = tdLoanTypeDescription;
                curData[6] = tdStatus;
  ...
  function EnabledDisbledText(vEnabled){
      ...
      $("#LoanType").attr('disabled', vEnabled); 
      $("#LoanTypeCode").attr('disabled', vEnabled); 
      $("#LoanTypeName").attr('disabled', vEnabled); 

      $("#Status").attr('disabled', vEnabled); 

      $("#Status").attr('disabled', vEnabled); 
      $("#LoanTypeDescription").attr('disabled', vEnabled); 
    ...
    function resetTextBorderToNormal(){
        ...
        $("#LoanTypeCode").css({"border":"#ccc 1px solid"});
        $("#LoanTypeName").css({"border":"#ccc 1px solid"}); 
        $("#LoanTypeDescription").css({"border":"#ccc 1px solid"}); 

        $("#LoanType").css({"border":"#ccc 1px solid"});   
        ...
        if(vRecordID > 0){
            $.ajax({
                ...
                data: {
                    _token: '{{ csrf_token() }}',
                    Platform: "{{ config('app.PLATFORM_ADMIN') }}",
                    LoanTypeID: vRecordID
                },
                url: "{{ route('get-loan-type-info') }}",
                dataType: "json",
                success: function(data){

                    if(data.Response =='Success' && data.LoanTypeInfo != undefined){

                        Clearfields();

                        $("#LoanTypeID").val(data.LoanTypeInfo.ID);
                        $("#LoanTypeCode").val(data.LoanTypeInfo.Code);
                        $("#LoanTypeName").val(data.LoanTypeInfo.Name);
                        $("#LoanTypeDescription").val(data.LoanTypeInfo.Description);
                        $("#LoanType").val(data.LoanTypeInfo.Type).change();
                        $("#Status").val(data.LoanTypeInfo.Status).change();

                        $("#LoanTypeCode").attr("style", "border: 1px solid rgb(204, 204, 204) !important");
                        $("#LoanTypeName").attr("style", "border: 1px solid rgb(204, 204, 204) !important");
                        $("#LoanTypeDescription").attr("style", "border: 1px solid rgb(204, 204, 204) !important");
                        $("#LoanType").attr("style", "border: 1px solid rgb(204, 204, 204) !important");
    ...
    function SaveRecord(){
        ...
        var vLoanTypeCode = $("#LoanTypeCode").val();
        var vLoanTypeName = $("#LoanTypeName").val();
        var vLoanTypeDescription = $("#LoanTypeDescription").val();
       
        var vLoanType = $("#LoanType").val();
        var vStatus = $("#Status").val();

        resetTextBorderToNormal();

        if(vLoanType.trim()=="") {
           showHasErrorMessage('LoanType','Select taxable loan type from the list.');
           return;
       }

        if(vLoanTypeCode.trim()=="") {
           showHasErrorMessage('LoanTypeCode','Select loan type from the list.');
           return;
       }

        if(vLoanTypeName.trim()=="") {
           showHasErrorMessage('LoanTypeName','Enter loan type name.');
           return;
       }

        if(vLoanTypeDescription.trim()=="") {
           showHasErrorMessage('LoanTypeDescription','Enter loan type description.');
           return;
       }
       ...
       // SAVE DATA
        $.ajax({
            ...
            data: {
                ...
                LoanTypeName: $("#LoanTypeName").val(),
                LoanTypeDescription: $("#LoanTypeDescription").val(),
                Status: $("#Status").val()
            },
            url: "{{ route('do-save-loan-type') }}",
            dataType: "json",
            success: function(data){
                ...
                if(data.Response =='Success'){
                    ...
                    LoadRecordRow(data.LoanTypeInfo);
                }else{
                    showHasErrorMessage('',data.ResponseMessage);
                    return;
  ...
  function GenerateExcel(){
    $.ajax({
          ...
          url: "{{ route('get-excel-loan-type-list') }}",
          dataType: "json",
          success: function(data){

              if(data.Response=="Success"){
                   resultquery=data.AllLoanTypeList;
                   ShowGeneratedExcel();
              }
  ...
  function ShowGeneratedExcel(){
         ...
         // Excel Headers
          var xlsHeader = [
            ...
            XLSX.utils.book_append_sheet(wb, ws,"Loan Type List Sheet");

            /* generate file and download */
            const wbout = XLSX.write(wb, { type: "array", bookType: "xlsx" });
            saveAs(new Blob([wbout], { type: "application/octet-stream" }), "All-Loan-Type-Report.xlsx");
