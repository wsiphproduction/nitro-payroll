                        ...
                        <div class="col-12">
                            <div class="breadcrumb-wrapper col-12">
                                <ol class="breadcrumb p-0 mb-0">
                                    ...
                                    <li class="breadcrumb-item active">PHIC Table List
                                    </li>
                                </ol>
                            </div>
                        </div>
                        ...
                        <div class="col-md-12">
                            <div class="card">
                                <div class="card-header">
                                    <h4 class="card-title">PHIC Table </h4>
                                </div>
        ...
        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal" role="document">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <h5 class="modal-title white-color">PHIC Table Information</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <i class="bx bx-x"></i>
                    </button>
                </div>
                <div class="modal-body">
                    <input type="hidden" id="PHIC_ID" value="0" readonly>
                    ...
                    <div class="row">
                     ...
                     <div class="row">
                        <div class="col-md-12">
                            <fieldset class="form-group">
                                <label for="TotalSharePercent">Total Share %: <span class="required_field">*</span></label>
                                <div class="div-percent">
                                    <input id="TotalSharePercent" type="text" class="form-control DecimalOnly text-align-right" placeholder="Total Share Percent" autocomplete="off"> <span class='percent-sign'> %</span>
                                </div> 
    ...
    <div id="upload-modal" class="modal fade text-left w-100" role="dialog" aria-labelledby="myModalLabel16" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal" role="document">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <h5 class="modal-title white-color">PHIC Excel Uploader</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <i class="bx bx-x"></i>
                    </button>
                </div>
                <div class="modal-body">
                    <div class="row">
                          <h5>Browse & select PHIC table excel file:</h5>
                         <fieldset class="form-group">
...
<script type="text/javascript">
    ...
    function getRecordList(vPageNo){
      ...
      vLimit=100;
        ...
        $.ajax({
            ...
            url: "{{ route('get-phic-table-bracket-list') }}",
            dataType: "json",
            success: function(data){
                total_rec=data.TotalRecord;
                LoadRecordList(data.PHICTableBracketList);
                 if(total_rec>0){
                     CreatePHICTableBracketPaging(total_rec,vLimit);  
                     if(total_rec>vLimit){
                        $("#divPaging").show(); 
    ...
    function CreatePHICTableBracketPaging(vTotalRecord,vLimit){
    ...
    function LoadRecordRow(vData){
        ...
        tdTotalSharePercent = "<span>" + vData.TotalSharePercent + " % </span>";
    ...
    function EnabledDisbledText(vEnabled){
        ...
        $("#TotalSharePercent").attr('disabled', vEnabled); 
    ...
    function resetTextBorderToNormal(){
        ...
        $("#TotalSharePercent").css({"border":"#ccc 1px solid"});  
    ...
    function EditRecord(vRecordID,vAllowEdit){
        ...
        if(vRecordID > 0){
            $.ajax({
                ...
                url: "{{ route('get-phic-table-bracket-info') }}",
                dataType: "json",
                success: function(data){

                    if(data.Response =='Success' && data.PHICTableBracketInfo != undefined){

                        Clearfields();

                        $("#PHIC_ID").val(data.PHICTableBracketInfo.ID);

                        $("#SalaryFrom").val(FormatDecimal(data.PHICTableBracketInfo.RangeFrom,2));
                        $("#SalaryTo").val(FormatDecimal(data.PHICTableBracketInfo.RangeTo,2));
                        
                        $("#TotalSharePercent").val(data.PHICTableBracketInfo.TotalSharePercent);
                        $("#Year").val(data.PHICTableBracketInfo.Year).change();
                        $("#Status").val(data.PHICTableBracketInfo.Status).change();
                        ...
                        $("#TotalSharePercent").attr("style", "border: 1px solid rgb(204, 204, 204) !important");
    ...
    function SaveRecord(){
        ...
        var vPHIC_ID = $("#PHIC_ID").val();
        var vSalaryFrom = $("#SalaryFrom").val();
        var vSalaryTo = $("#SalaryTo").val();
        var vTotalSharePercent = $("#TotalSharePercent").val();
        ...
        if(vYear=="") {
         showHasErrorMessage('Year','Select PHIC year applied from the list.');
         return;  
       }
        ...
        if(vSalaryFrom=="") {
         showHasErrorMessage('SalaryFrom','Enter PHIC salary range from.');
         return;  
       }

       if(vSalaryTo=="") {
         showHasErrorMessage('SalaryTo','Enter PHIC salary range to.');
         return;  
       }

        if(vTotalSharePercent=="") {
         showHasErrorMessage('TotalSharePercent','Enter PHIC total share percentage');
         return;  
       }
      ...
      // SAVE DATA
        $.ajax({
            ...
            data: {
                ...
                TotalSharePercent: $("#TotalSharePercent").val(),
                Year: $("#Year").val(),
                Status: $("#Status").val()
            },
            url: "{{ route('do-phic-table-bracket') }}",
            dataType: "json",
            success: function(data){
                ...
                if(data.Response =='Success'){
                    ...
                    LoadRecordRow(data.PHICTableBracketInfo);
                }else{
                    showHasErrorMessage('',data.ResponseMessage);
                    return;  
                }
   ...
   function GenerateExcel(){
    $.ajax({
          ...
          url: "{{ route('get-excel-phic-bracket-list') }}",
          dataType: "json",
          success: function(data){

              if(data.Response=="Success"){
                   resultquery=data.AllPHICBracketList;
                   ShowGeneratedExcel();
              }
  ...
  function ShowGeneratedExcel(){
         ...
         // Excel Headers
          var xlsHeader = [
                            " Salary From "," Salary To "," Total Share % ", 
                            " Year "," Status "                            
                          ];
            ...
            XLSX.utils.book_append_sheet(wb, ws,"PHIC Bracket List Sheet");

            /* generate file and download */
            const wbout = XLSX.write(wb, { type: "array", bookType: "xlsx" });
            saveAs(new Blob([wbout], { type: "application/octet-stream" }), "All-PHIC-Bracket-Report.xlsx");
